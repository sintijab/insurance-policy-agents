...
import { workflowDataMultiPolicies } from "../lib/workflowData"
      ...
      <div className="sticky bottom-0 border-t p-4 space-y-4">
        <div className="flex items-end gap-2">
          <Button onClick={() => {
            ...
            <Sparkles className="h-2 w-2" /> Verify account holders with multiple policies
          </Button>
        </div>
        <div className="flex items-end gap-2">
          <Textarea
    ...
    <div className="absolute left-0 top-0 flex h-full w-96 flex-col border-r bg-background shadow-lg">
      ...
      {runId && <div className="m-4 p-4 text-xs bg-muted-foreground/10 rounded-md overflow-hidden space-y-2">
        <div className="flex items-center justify-between gap-2">
          ...
          <Link href={`https://ref8nwsq.clj5khk.gcp.restack.it/runs/${agentId}/${runId}?scheduledEventId=1`} target="_blank" className="text-xs underline text-muted-foreground hover:text-foreground">Open on Restack</Link>
        </div>
        <pre>{JSON.stringify({agentId, runId}, null, 2)}</pre>
        
      </div>}
      <ChatMessages agentId={agentId} runId={runId} currentFlow={currentFlow} workflowData={workflowDataMultiPolicies} setNodes={setNodes} setEdges={setEdges} />
    </div>
  )
}
